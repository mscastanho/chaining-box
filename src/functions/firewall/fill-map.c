...
const char* mappath = "/sys/fs/bpf/tc/globals/rules";
...
int main(int argc __attribute__ ((unused)), const char **argv)
{
  ...
  err = kefir_cprog_fill_pinned_map(cprog, mappath);
  if (err){
    fprintf(stderr,"Check that you're running as root and the program is already loaded.\n");
    goto destroy_stuff;
  }
